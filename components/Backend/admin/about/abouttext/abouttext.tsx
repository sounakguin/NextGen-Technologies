...
import AddAboutText from "./addabouttext";
import EditAboutText from "./editabouttext";
import ViewAboutText from "./viewabouttext";
...
export default function AboutText() {
  const [items, setItems] = useState<AboutTextItem[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);
  const [currentItem, setCurrentItem] = useState<AboutTextItem | null>(null);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const { error } = await supabase.from("abouttext").delete().eq("id", id);
