...
export default function AddSaaSApp({ onClose, onAdd }: AddSaaSAppProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      // Insert SaaS App
      const { data: saasAppData, error: saasAppError } = await supabase
        .from("saas_app")
        .insert([{ title, description, service_id: selectedServiceId, slug }])
        .select();

      if (saasAppError) throw saasAppError;

      const saasAppId = saasAppData[0].id;
      ...
      console.error("Error creating SaaS App:", err);
    }
  };
  ...
  const handleTemplateChange = (
    index: number,
    field: keyof SaaSAppTemplateDetail,
    ...
    <div className="fixed inset-0 flex justify-center items-center bg-black/50">
      <div className="bg-white p-6 rounded-2xl shadow-xl w-full sm:w-[800px] max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">Add SaaS App</h2>
          <button
            onClick={onClose}
