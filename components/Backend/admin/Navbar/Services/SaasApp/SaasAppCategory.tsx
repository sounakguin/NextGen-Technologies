...
export default function SaaSAppCategory({ onClose }: { onClose: () => void }) {
  const [categories, setCategories] = useState<SaaSAppCategory[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [currentCategory, setCurrentCategory] =
    useState<SaaSAppCategory | null>(null);
  ...
  const handleEditCategory = (category: SaaSAppCategory) => {
