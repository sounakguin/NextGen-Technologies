...
export default function MicroSaasCategory({
  onClose,
}: {
  onClose: () => void;
}) {
  const [categories, setCategories] = useState<MicroSaasCategory[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [currentCategory, setCurrentCategory] =
    useState<MicroSaasCategory | null>(null);
  ...
  const handleEditCategory = (category: MicroSaasCategory) => {
