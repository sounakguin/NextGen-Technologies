...
export default function AddWebTemplate({
  onClose,
  onAdd,
}: AddWebTemplateProps) {
  ...
  const handleCreate = async () => {
    ...
    try {
      // Insert web template
      const { data: webTemplateData, error: webTemplateError } = await supabase
        .from("web_template")
      ...
      const webTemplateId = webTemplateData[0].id;
      ...
      console.error("Error creating web template:", err);
    }
  };
