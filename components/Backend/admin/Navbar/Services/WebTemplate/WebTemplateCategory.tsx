...
export default function WebTemplateCategory({
  onClose,
}: {
  onClose: () => void;
}) {
  const [categories, setCategories] = useState<WebTemplateCategory[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isEditing, setIsEditing] = useState(false);
  const [currentCategory, setCurrentCategory] =
    useState<WebTemplateCategory | null>(null);
  ...
  const handleEditCategory = (category: WebTemplateCategory) => {
